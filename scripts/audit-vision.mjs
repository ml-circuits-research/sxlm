...
export function auditVision({ model = new SymbolicModel() } = {}) {
  ...
  const reproduced = new Map([...rebuilt.circuits,...semantics.circuits,...sop.circuits,...sequenceCircuits,...learnedPrograms,...(planningMethodReplay?graphsFromModules(planning.learned):[]),...(lexicalMethodReplay?graphsFromModules(lexical.learned):[])].filter(c=>c.learning).map(c=>[c.id, executionDigest(c)]));
